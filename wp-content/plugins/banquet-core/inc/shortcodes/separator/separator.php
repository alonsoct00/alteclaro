...
if ( ! function_exists( 'banquet_core_add_separator_shortcode' ) ) {
	/**
	 ...
	 * @return array
	 */
	function banquet_core_add_separator_shortcode( $shortcodes ) {
		$shortcodes[] = 'BanquetCoreSeparatorShortcode';
		
		return $shortcodes;
	}
	
	add_filter( 'banquet_core_filter_register_shortcodes', 'banquet_core_add_separator_shortcode', 9 );
}

if ( class_exists( 'BanquetCoreShortcode' ) ) {
	class BanquetCoreSeparatorShortcode extends BanquetCoreShortcode {
		
		public function map_shortcode() {
			$this->set_shortcode_path( BANQUET_CORE_SHORTCODES_URL_PATH . '/separator' );
			$this->set_base( 'banquet_core_separator' );
			$this->set_name( esc_html__( 'Separator', 'banquet-core' ) );
			$this->set_description( esc_html__( 'Shortcode that displays separator with provided parameters', 'banquet-core' ) );
			...
			$this->set_option( array(
				'field_type' => 'select',
				'name'       => 'position',
				'title'      => esc_html__( 'Position', 'banquet-core' ),
				'options'    => array(
			...
			$this->set_option( array(
				'field_type' => 'select',
				'name'       => 'border_style',
				'title'      => esc_html__( 'Border Style', 'banquet-core' ),
				'options'    => banquet_core_get_select_type_options_pool( 'border_style' ),
				'group'      => esc_html__( 'Style', 'banquet-core' )
			) );
			$this->set_option( array(
				'field_type' => 'text',
				'name'       => 'width',
				'title'      => esc_html__( 'Width (px or %)', 'banquet-core' ),
				'group'      => esc_html__( 'Style', 'banquet-core' )
			) );
			$this->set_option( array(
				'field_type' => 'text',
				'name'       => 'thickness',
				'title'      => esc_html__( 'Thickness (px)', 'banquet-core' ),
				'group'      => esc_html__( 'Style', 'banquet-core' )
			) );
			$this->set_option( array(
				'field_type' => 'text',
				'name'       => 'margin_top',
				'title'      => esc_html__( 'Margin Top (px or %)', 'banquet-core' ),
				'group'      => esc_html__( 'Style', 'banquet-core' )
			) );
			$this->set_option( array(
				'field_type' => 'text',
				'name'       => 'margin_bottom',
				'title'      => esc_html__( 'Margin Bottom (px or %)', 'banquet-core' ),
				'group'      => esc_html__( 'Style', 'banquet-core' )
			) );
		}
		...
		public function render( $options, $content = null ) {
			...
			return banquet_core_get_template_part( 'shortcodes/separator', 'templates/separator', '', $atts );
		}
