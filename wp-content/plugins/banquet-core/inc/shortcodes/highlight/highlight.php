...
if ( ! function_exists( 'banquet_core_add_highlight_shortcode' ) ) {
	/**
	 ...
	 * @return array
	 */
	function banquet_core_add_highlight_shortcode( $shortcodes ) {
		$shortcodes[] = 'BanquetCoreHighlightShortcode';

		return $shortcodes;
	}

	add_filter( 'banquet_core_filter_register_shortcodes', 'banquet_core_add_highlight_shortcode' );
}

if ( class_exists( 'BanquetCoreShortcode' ) ) {
	class BanquetCoreHighlightShortcode extends BanquetCoreShortcode {

		public function map_shortcode() {
			$this->set_shortcode_path( BANQUET_CORE_SHORTCODES_URL_PATH . '/highlight' );
			$this->set_base( 'banquet_core_highlight' );
			$this->set_name( esc_html__( 'Highlight', 'banquet-core' ) );
			$this->set_description( esc_html__( 'Shortcode that displays highlight with provided parameters', 'banquet-core' ) );
			...
			$this->set_option( array(
				'field_type'    => 'select',
				'name'          => 'text_tag',
				'title'         => esc_html__( 'Text Tag', 'banquet-core' ),
			...
			$this->set_option( array(
				...
				'title'       => esc_html__( 'Positions of Line Break', 'banquet-core' ),
				'description' => esc_html__( 'Enter the positions of the words after which you would like to create a line break. Separate the positions with commas (e.g. if you would like the first, third, and fourth word to have a line break, you would enter "1,3,4")', 'banquet-core' )
			) );
			$this->set_option( array(
				'field_type'    => 'select',
				'name'          => 'disable_title_break_words',
				'title'         => esc_html__( 'Disable Text Line Break', 'banquet-core' ),
				'description'   => esc_html__( 'Enabling this option will disable text line breaks for screen size 1024 and lower', 'banquet-core' ),
			...
			$this->set_option( array(
				...
				'title'       => esc_html__( 'Highlight Text Position', 'banquet-core' ),
				'description' => esc_html__( 'Enter the positions of the words you would like to display as "highlight" text. Separate start and end positions with commas (e.g. if you would like to wrap from fifth to ninth words, you would enter "5,9") or if you want to highlight whole text fill -1', 'banquet-core' ),
			) );
			$this->set_option( array(
				'field_type' => 'color',
				'name'       => 'highlight_color',
				'title'      => esc_html__( 'Highlight Text Color', 'banquet-core' )
			) );
			$this->set_option( array(
				'field_type' => 'color',
				'name'       => 'highlight_background_color',
				'title'      => esc_html__( 'Highlight Text Background Color', 'banquet-core' )
			) );
			$this->set_option( array(
				'field_type'    => 'text',
				'name'          => 'link',
		...
		public function render( $options, $content = null ) {
			...
			return banquet_core_get_template_part( 'shortcodes/highlight', 'templates/highlight', '', $atts );
		}
