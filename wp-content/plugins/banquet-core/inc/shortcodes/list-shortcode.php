...
abstract class BanquetCoreListShortcode extends QodeFrameworkShortcode {
	private $post_type;
	...
	public function map_list_options( $params = array() ) {
		...
		$this->set_option( array(
			'field_type'    => 'select',
			'name'          => 'behavior',
			'title'         => esc_html__( 'List Behavior', 'banquet-core' ),
			'options'       => banquet_core_get_select_type_options_pool( 'list_behavior', false, $exclude_behavior ),
		...
		if ( empty( $exclude_behavior ) || ( ! empty( $exclude_behavior ) && ! in_array( 'masonry', $exclude_behavior ) ) ) {
			$this->set_option( array(
				...
				'title'         => esc_html__( 'Images Proportion', 'banquet-core' ),
				'options'       => array(
					''      => esc_html__( 'Original', 'banquet-core' ),
		...
		if ( empty( $exclude_option ) || ( ! empty( $exclude_option ) && ! in_array( 'images_proportion', $exclude_option ) ) ) {
			$this->set_option( array(
				...
				'default_value' => 'full',
				'title'         => esc_html__( 'Images Proportion', 'banquet-core' ),
				'options'       => banquet_core_get_select_type_options_pool( 'list_image_dimension', false ),
				'group'         => $group,
				'dependency'    => array(
					'show' => array(
						'behavior' => array(
			...
			$this->set_option( array(
				...
				'title'         => esc_html__( 'Custom Image Width', 'banquet-core' ),
				'description'   => esc_html__( 'Enter image width in px', 'banquet-core' ),
				'group'         => $group,
				'dependency'    => array(
					'show' => array(
			...
			$this->set_option( array(
				...
				'title'         => esc_html__( 'Custom Image Height', 'banquet-core' ),
				'description'   => esc_html__( 'Enter image height in px', 'banquet-core' ),
				'group'         => $group,
				'dependency'    => array(
					'show' => array(
						'images_proportion' => array(
		...
		$this->set_option( array(
			'field_type'    => 'select',
			'name'          => 'columns',
			'title'         => esc_html__( 'Columns Number', 'banquet-core' ),
			'options'       => banquet_core_get_select_type_options_pool( 'columns_number', true, $exclude_columns, $include_columns ),
			'default_value' => '3',
			'group'         => $group,
			'dependency'    => array(
				'hide' => array(
		...
		$this->set_option( array(
			'field_type'    => 'select',
			'name'          => 'space',
			'title'         => esc_html__( 'Space Between Items', 'banquet-core' ),
		...
		$this->set_option( array(
			...
			'title'         => esc_html__( 'Columns Responsive', 'banquet-core' ),
			'options'       => banquet_core_get_select_type_options_pool( 'columns_responsive' ),
		...
		$this->set_option( array(
			'field_type'    => 'select',
			'name'          => 'columns_1440',
			'title'         => esc_html__( 'Columns Number 1367px - 1440px', 'banquet-core' ),
			'options'       => banquet_core_get_select_type_options_pool( 'columns_number' ),
			'default_value' => '3',
			'dependency' => array(
				'show' => array(
		...
		$this->set_option( array(
			'field_type'    => 'select',
			'name'          => 'columns_1366',
			'title'         => esc_html__( 'Columns Number 1025px - 1366px', 'banquet-core' ),
			'options'       => banquet_core_get_select_type_options_pool( 'columns_number' ),
			'default_value' => '3',
			'dependency' => array(
				'show' => array(
		...
		$this->set_option( array(
			'field_type'    => 'select',
			'name'          => 'columns_1024',
			'title'         => esc_html__( 'Columns Number 769px - 1024px', 'banquet-core' ),
			'options'       => banquet_core_get_select_type_options_pool( 'columns_number' ),
			'default_value' => '3',
			'dependency' => array(
				'show' => array(
		...
		$this->set_option( array(
			'field_type'    => 'select',
			'name'          => 'columns_768',
			'title'         => esc_html__( 'Columns Number 681px - 768px', 'banquet-core' ),
			'options'       => banquet_core_get_select_type_options_pool( 'columns_number' ),
			'default_value' => '3',
			'dependency' => array(
				'show' => array(
		...
		$this->set_option( array(
			'field_type'    => 'select',
			'name'          => 'columns_680',
			'title'         => esc_html__( 'Columns Number 481px - 680px', 'banquet-core' ),
			'options'       => banquet_core_get_select_type_options_pool( 'columns_number' ),
			'default_value' => '3',
			'dependency' => array(
				'show' => array(
		...
		$this->set_option( array(
			'field_type'    => 'select',
			'name'          => 'columns_480',
			'title'         => esc_html__( 'Columns Number 0 - 480px', 'banquet-core' ),
			'options'       => banquet_core_get_select_type_options_pool( 'columns_number' ),
			'default_value' => '3',
			'dependency' => array(
				'show' => array(
		...
		if ( empty( $exclude_behavior ) || ( ! empty( $exclude_behavior ) && ! in_array( 'justified-gallery', $exclude_behavior ) ) ) {
			$this->set_option( array(
				...
				'title'      => esc_html__( 'Row Height', 'banquet-core' ),
				'dependency' => array(
					'show' => array(
						'behavior' => array(
							'values'        => 'justified-gallery',
			...
			$this->set_option( array(
				...
				'title'      => esc_html__( 'Max Row Height', 'banquet-core' ),
				'dependency' => array(
					'show' => array(
						'behavior' => array(
							'values'        => 'justified-gallery',
			...
			$this->set_option( array(
				...
				'title'      => esc_html__( 'Last Row Treshold', 'banquet-core' ),
				'dependency' => array(
					'show' => array(
						'behavior' => array(
							'values'        => 'justified-gallery',
	...
	public function map_slider_options( $params = array() ) {
		...
		$this->set_option( array(
			'field_type' => 'select',
			'name'       => 'slider_loop',
			'title'      => esc_html__( 'Enable Slider Loop', 'banquet-core' ),
		...
		$this->set_option( array(
			...
			'title'      => esc_html__( 'Enable Slider Autoplay', 'banquet-core' ),
			'options'    => banquet_core_get_select_type_options_pool( 'yes_no' ),
			'dependency' => $dependency,
			'group'      => $group
		) );
		$this->set_option( array(
			'field_type'  => 'text',
			'name'        => 'slider_speed',
			'title'       => esc_html__( 'Slide Duration', 'banquet-core' ),
			'description' => esc_html__( 'Default value is 5000 (ms)', 'banquet-core' ),
			'dependency'  => $dependency,
			'group'       => $group
		) );
		$this->set_option( array(
			...
			'title'       => esc_html__( 'Slide Animation Duration', 'banquet-core' ),
			'description' => esc_html__( 'Speed of slide animation in milliseconds. Default value is 800.', 'banquet-core' ),
			'dependency'  => $dependency,
			'group'       => $group
		) );
		$this->set_option( array(
			...
			'title'      => esc_html__( 'Enable Slider Navigation', 'banquet-core' ),
			'options'    => banquet_core_get_select_type_options_pool( 'yes_no' ),
			'dependency' => $dependency,
			'group'      => $group
		) );
		$this->set_option( array(
			...
			'title'      => esc_html__( 'Enable Slider Pagination', 'banquet-core' ),
			'options'    => banquet_core_get_select_type_options_pool( 'yes_no' ),
			'dependency' => $dependency,
			'group'      => $group
		) );
	...
	public function get_list_item_image_dimension( $atts ) {
		...
		if ( ! empty( $atts['behavior'] ) && $atts['behavior'] == 'masonry' && ! empty( $atts['masonry_images_proportion'] ) && $atts['masonry_images_proportion'] == 'fixed' ) {
			...
			$image_dimension              = banquet_core_get_custom_image_size_meta( 'meta-box', $masonry_image_dimension_name, get_the_ID() );
		}
	...
	public function get_slider_data( $atts, $include = array() ) {
		...
		$data['spaceBetween'] = isset( $atts['space'] ) ? banquet_core_get_space_value( $atts['space'] ) : 0;
	...
	public function map_query_options( $params = array() ) {
		...
		$this->set_option( array(
			...
			'options'       => banquet_core_get_select_type_options_pool( 'order_by', true, $exclude_order_by, $include_order_by ),
			'default_value' => 'date',
			'group'         => $group
		) );
		$this->set_option( array(
		...
		$this->set_option( array(
			...
			'options'    => array(
				...
				'id'  => esc_html__( 'Post IDs', 'banquet-core' ),
				'tax' => esc_html__( 'Taxonomy Slug', 'banquet-core' ),
			),
			'group'      => $group
		) );
		$this->set_option( array(
			'field_type' => 'text',
			'name'       => 'post_ids',
			'title'      => esc_html__( 'Posts IDs (separate with comma)', 'banquet-core' ),
			'group'      => $group,
		    'dependency' => array(
				'show' => array(
		...
		if( ! empty( $taxonomies_formatted ) ) {
			$this->set_option( array(
				'field_type'    => 'select',
				'name'          => 'tax',
				'title'         => esc_html__( 'Taxonomy', 'banquet-core' ),
			...
			$this->set_option( array(
				'field_type' => 'text',
				'name'       => 'tax_slug',
				'title'      => esc_html__( 'Taxonomy Slug', 'banquet-core' ),
				'group'      => $group,
				'dependency' => array(
					'show' => array(
						'additional_params' => array(
			...
			$this->set_option( array(
				'field_type'  => 'text',
				'name'        => 'tax__in',
				'title'       => esc_html__( 'Taxonomy Ids', 'banquet-core' ),
				'description' => esc_html__( 'Separate taxonomy ids with commas', 'banquet-core' ),
				'group'       => $group,
				'dependency'  => array(
					'show' => array(
						'additional_params' => array(
	...
	public function map_layout_options( $params = array() ) {
		...
		$this->set_option( array(
			'field_type'    => 'select',
			'name'          => 'layout',
			'title'         => esc_html__( 'Item Layout', 'banquet-core' ),
			'options'       => $layouts,
		...
		if ( empty( $exclude_option ) || ( ! empty( $exclude_option ) && ! in_array( 'text_transform', $exclude_option ) ) ) {
			$this->set_option( array(
				'field_type' => 'select',
				'name'       => 'text_transform',
				'title'      => esc_html__( 'Title Text Transform', 'banquet-core' ),
	...
	public function map_additional_options( $params = array() ) {
		...
		if ( ! $exclude_filter ) {
			$this->set_option( array(
				'field_type'    => 'select',
				'name'          => 'enable_filter',
				'title'         => esc_html__( 'Enable Filter', 'banquet-core' ),
				'description'   => esc_html__( 'Enabling this option will show categories filer feature above list', 'banquet-core' ),
				'options'       => banquet_core_get_select_type_options_pool( 'no_yes' ),
				'default_value' => '',
				'group'         => esc_html__( 'Additional', 'banquet-core' )
			) );
		}
		...
		if ( ! $exclude_pagination ) {
			$this->set_option( array(
				...
				'title'         => esc_html__( 'Pagination', 'banquet-core' ),
				'options'       => banquet_core_get_select_type_options_pool( 'pagination_type' ),
			...
			$this->set_option( array(
				...
				'title'         => esc_html__( 'Load More Top Margin (px or %)', 'banquet-core' ),
				'default_value' => '',
				'group'         => esc_html__( 'Additional', 'banquet-core' ),
				'dependency' => array(
					'show' => array(
	...
	public function load_assets() {
		do_action( 'banquet_core_action_list_shortcodes_load_assets', $this->get_atts() );
	}
