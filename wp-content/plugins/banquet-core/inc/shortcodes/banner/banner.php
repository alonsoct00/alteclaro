...
if ( ! function_exists( 'banquet_core_add_banner_shortcode' ) ) {
	/**
	 ...
	 * @return array
	 */
	function banquet_core_add_banner_shortcode( $shortcodes ) {
		$shortcodes[] = 'BanquetCoreBannerShortcode';
		
		return $shortcodes;
	}
	
	add_filter( 'banquet_core_filter_register_shortcodes', 'banquet_core_add_banner_shortcode' );
}

if ( class_exists( 'BanquetCoreShortcode' ) ) {
	class BanquetCoreBannerShortcode extends BanquetCoreShortcode {
		
		public function __construct() {
			$this->set_layouts( apply_filters( 'banquet_core_filter_banner_layouts', array() ) );
			$this->set_extra_options( apply_filters( 'banquet_core_filter_banner_extra_options', array() ) );
			
			parent::__construct();
		}
		
		public function map_shortcode() {
			$this->set_shortcode_path( BANQUET_CORE_SHORTCODES_URL_PATH . '/banner' );
			$this->set_base( 'banquet_core_banner' );
			$this->set_name( esc_html__( 'Banner', 'banquet-core' ) );
			$this->set_description( esc_html__( 'Shortcode that adds banner element', 'banquet-core' ) );
			...
			$this->set_option( array(
				...
				'title'      => esc_html__( 'Subtitle Margin Top', 'banquet-core' ),
				'group'      => esc_html__( 'Content', 'banquet-core' )
			) );
			$this->set_option( array(
				'field_type' => 'text',
		...
		public function render( $options, $content = null ) {
			...
			return banquet_core_get_template_part( 'shortcodes/banner', 'variations/' . $atts['layout'] . '/templates/' . $atts['layout'], '', $atts );
		}
