...
if ( ! function_exists( 'banquet_core_add_icon_with_text_shortcode' ) ) {
	/**
	 ...
	 * @return array
	 */
	function banquet_core_add_icon_with_text_shortcode( $shortcodes ) {
		$shortcodes[] = 'BanquetCoreIconWithTextShortcode';
		
		return $shortcodes;
	}
	
	add_filter( 'banquet_core_filter_register_shortcodes', 'banquet_core_add_icon_with_text_shortcode' );
}

if ( class_exists( 'BanquetCoreShortcode' ) ) {
	class BanquetCoreIconWithTextShortcode extends BanquetCoreShortcode {
		
		public function __construct() {
			$this->set_layouts( apply_filters( 'banquet_core_filter_icon_with_text_layouts', array() ) );
			
			$options_map = banquet_core_get_variations_options_map( $this->get_layouts() );
			$default_value = $options_map['default_value'];
			
			$this->set_extra_options( apply_filters( 'banquet_core_filter_icon_with_text_extra_options', array(), $default_value ) );
			
			parent::__construct();
		}
		
		public function map_shortcode() {
			$this->set_shortcode_path( BANQUET_CORE_SHORTCODES_URL_PATH . '/icon-with-text' );
			$this->set_base( 'banquet_core_icon_with_text' );
			$this->set_name( esc_html__( 'Icon With Text', 'banquet-core' ) );
			$this->set_description( esc_html__( 'Shortcode that adds icon with text element', 'banquet-core' ) );
		...
		public function render( $options, $content = null ) {
			...
			return banquet_core_get_template_part( 'shortcodes/icon-with-text', 'variations/' . $atts['layout'] . '/templates/' . $atts['layout'], '', $atts );
		}
		...
		private function get_holder_classes( $atts ) {
			...
			$holder_classes = apply_filters( 'banquet_core_filter_icon_with_text_variation_classes', $holder_classes, $atts );
