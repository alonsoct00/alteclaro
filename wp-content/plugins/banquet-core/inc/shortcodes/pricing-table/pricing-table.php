...
if ( ! function_exists( 'banquet_core_add_pricing_table_shortcode' ) ) {
	/**
	 ...
	 * @return array
	 */
	function banquet_core_add_pricing_table_shortcode( $shortcodes ) {
		$shortcodes[] = 'BanquetCorePricingTableShortcode';
		
		return $shortcodes;
	}
	
	add_filter( 'banquet_core_filter_register_shortcodes', 'banquet_core_add_pricing_table_shortcode' );
}

if ( class_exists( 'BanquetCoreShortcode' ) ) {
	class BanquetCorePricingTableShortcode extends BanquetCoreShortcode {
		
		public function __construct() {
			$this->set_layouts( apply_filters( 'banquet_core_filter_pricing_table_layouts', array() ) );
			$this->set_extra_options( apply_filters( 'banquet_core_filter_pricing_table_extra_options', array() ) );
			
			parent::__construct();
		}
		
		public function map_shortcode() {
			$this->set_shortcode_path( BANQUET_CORE_SHORTCODES_URL_PATH . '/pricing-table' );
			$this->set_base( 'banquet_core_pricing_table' );
			$this->set_name( esc_html__( 'Pricing Table', 'banquet-core' ) );
			$this->set_description( esc_html__( 'Shortcode that adds pricing table element', 'banquet-core' ) );
			...
			$this->set_option( array(
				'field_type'    => 'select',
				'name'          => 'featured_table',
				'title'         => esc_html__( 'Featured Table', 'banquet-core' ),
			...
			$this->set_option( array(
				'field_type' => 'text',
				'name'       => 'price',
				'title'      => esc_html__( 'Price', 'banquet-core' )
			) );
			$this->set_option( array(
				'field_type' => 'color',
				'name'       => 'price_color',
				'title'      => esc_html__( 'Price Color', 'banquet-core' )
			) );
			$this->set_option( array(
				'field_type' => 'text',
				'name'       => 'currency',
				'title'      => esc_html__( 'Currency', 'banquet-core' )
			) );
			$this->set_option( array(
				'field_type' => 'color',
				'name'       => 'currency_color',
				'title'      => esc_html__( 'Currency Color', 'banquet-core' )
			) );
			$this->set_option( array(
				'field_type' => 'html',
				'name'       => 'content',
		...
		public function render( $options, $content = null ) {
			...
			return banquet_core_get_template_part( 'shortcodes/pricing-table', 'variations/' . $atts['layout'] . '/templates/' . $atts['layout'], '', $atts );
		}
