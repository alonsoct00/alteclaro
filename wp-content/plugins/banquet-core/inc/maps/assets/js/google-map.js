	...
	var qodefGoogleMap = {
		...
		initMapCenter: function($map, addressesLatLng) {
			...
			for (var index = 0; index < addressesLatLng.length; ++index) {
				lat_min = addressesLatLng[index].lat < lat_min ? addressesLatLng[index].lat : lat_min;
				lat_max = addressesLatLng[index].lat > lat_max ? addressesLatLng[index].lat : lat_max;
				
				lng_min = addressesLatLng[index].lng < lng_min ? addressesLatLng[index].lng : lng_min;
				lng_max = addressesLatLng[index].lng > lng_max ? addressesLatLng[index].lng : lng_max;
			}
			$map.setCenter(new google.maps.LatLng(
				((lat_max + lat_min) / 2.0),
				((lng_max + lng_min) / 2.0)
			));
			// This part of code disable zoom setting
