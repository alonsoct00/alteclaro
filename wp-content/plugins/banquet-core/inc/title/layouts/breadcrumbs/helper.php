...
if ( ! function_exists( 'banquet_core_register_breadcrumbs_title_layout' ) ) {
	function banquet_core_register_breadcrumbs_title_layout( $layouts ) {
		$layouts['breadcrumbs'] = 'BanquetCoreBreadcrumbsTitle';
		
		return $layouts;
	}
	
	add_filter( 'banquet_core_filter_register_title_layouts', 'banquet_core_register_breadcrumbs_title_layout');
}

if ( ! function_exists( 'banquet_core_add_breadcrumbs_title_layout_option' ) ) {
	/**
	 ...
	 * @return array
	 */
	function banquet_core_add_breadcrumbs_title_layout_option( $layouts ) {
		$layouts['breadcrumbs'] = esc_html__( 'Breadcrumbs', 'banquet-core' );
		
		return $layouts;
	}
	
	add_filter( 'banquet_core_filter_title_layout_options', 'banquet_core_add_breadcrumbs_title_layout_option' );
}

if ( ! function_exists( 'banquet_core_breadcrumbs' ) ) {
	/**
	 * Function that renders breadcrumbs html
	 */
	function banquet_core_breadcrumbs() {
		
		// Breadcrumbs label
		$labels = apply_filters( 'banquet_core_filter_breadcrumbs_label', array(
			'home'        => esc_html__( 'Home', 'banquet-core' ),
			'tag'         => esc_html__( 'Posts tagged "%s"', 'banquet-core' ),
			'author'      => esc_html__( 'Posted by %s', 'banquet-core' ),
			'search'      => esc_html__( 'Search results for "%s"', 'banquet-core' ),
			'404'         => esc_html__( '404 - Page not found', 'banquet-core' ),
			'query_paged' => esc_html__( '(Page %s)', 'banquet-core' )
		) );
		
		// Breadcrumbs variables
		$settings = apply_filters( 'banquet_core_filter_breadcrumbs_settings', array(
		...
		$breadcrumbs_html = '';
		if ( ! empty( $wrap ) ) {
			$breadcrumbs_html = $settings['wrap_before'] . $wrap . apply_filters( 'banquet_core_filter_breadcrumbs_content', $wrap_child, $settings ) . $settings['wrap_after'];
		}
		
		echo apply_filters( 'banquet_core_filter_breadcrumbs_template', $breadcrumbs_html );
	}
}