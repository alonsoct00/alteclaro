...
if ( ! function_exists( 'banquet_core_add_page_title_options' ) ) {
	/**
	 * Function that add general options for this module
	 */
	function banquet_core_add_page_title_options() {
		...
		$page = $qode_framework->add_options_page(
			array(
				...
				'description' => esc_html__( 'Global settings related to title', 'banquet-core' )
			)
		);
		...
		if ( $page ) {
			...
			$page_title_section->add_field_element(
				array(
					...
					'options'     	=> apply_filters( 'banquet_core_filter_title_layout_options', array() ),
			...
			$page_title_section->add_field_element(
				array(
					...
					'options'       => banquet_core_get_select_type_options_pool( 'title_tag', false ),
					'default_value' => 'h1'
				)
			);
			...
			do_action( 'banquet_core_action_after_page_title_options_map', $page_title_section );
		}
	}
	
	add_action( 'banquet_core_action_default_options_init', 'banquet_core_add_page_title_options', banquet_core_get_admin_options_map_position( 'title' ) );
}