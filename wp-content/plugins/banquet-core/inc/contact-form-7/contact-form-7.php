...
if ( ! class_exists( 'BanquetCoreContactForm7' ) ) {
	class BanquetCoreContactForm7 {
		...
		function include_widgets() {
			foreach ( glob( BANQUET_CORE_INC_PATH . '/contact-form-7/widgets/*/include.php' ) as $widget ) {
				include_once $widget;
			}
		}
		...
		function override_templates() {
			...
			add_action( 'wpcf7_init', 'banquet_core_cf7_add_submit_form_tag' );
		}
	}
	
	BanquetCoreContactForm7::get_instance();
}