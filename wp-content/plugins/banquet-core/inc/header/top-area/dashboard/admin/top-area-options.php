...
if ( ! function_exists( 'banquet_core_add_top_area_options' ) ) {
	function banquet_core_add_top_area_options( $page ) {

		$top_area_section = $page->add_section_element(
			array(
				'name'        => 'qodef_top_area_section',
				'title'       => esc_html__( 'Top Area', 'banquet-core' ),
				'description' => esc_html__( 'Options related to top area', 'banquet-core' ),
				'dependency' => array(
					'hide' => array(
	...
	add_action( 'banquet_core_action_after_header_options_map', 'banquet_core_add_top_area_options');
}