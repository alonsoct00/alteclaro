...
abstract class BanquetCoreHeader {
	...
	public function __construct() {
		...
		add_filter( 'banquet_core_action_before_main_css', array( $this, 'enqueue_assets' ) );
		add_filter( 'banquet_core_action_before_main_css', array( $this, 'enqueue_additional_assets' ) );
		add_filter( 'banquet_filter_localize_main_js', array( $this, 'set_global_javascript_variables' ) );
		add_filter( 'banquet_core_filter_content_margin', array( $this, 'get_content_margin' ) );
		add_filter( 'banquet_core_filter_title_padding', array( $this, 'get_title_padding' ) );
		add_filter( 'banquet_filter_add_inline_style', array( $this, 'set_inline_header_styles' ) );
		add_filter( 'banquet_filter_header_inner_class', array( $this, 'set_grid_class' ) );
	}
	
	public function load_template( $parameters = array() ) {
		$parameters = apply_filters( 'banquet_core_filter_header_template', $parameters );
		
		return banquet_core_get_template_part( 'header/layouts/' . $this->slug, 'templates/' . $this->slug, '', $parameters );
	}
	...
	public function get_header_transparency() {
		$background_color = banquet_core_get_post_value_through_levels( 'qodef_' . $this->slug . '_header_background_color' );
	...
	function set_header_height() {
		$header_height = banquet_core_get_post_value_through_levels( 'qodef_' . $this->slug . '_header_height' );
		$header_height = ! empty( $header_height ) ? intval( $header_height ) : $this->default_header_height;
		
		$this->header_height = apply_filters( 'banquet_core_filter_set_header_height', $header_height );
	}
	
	function set_grid_class( $class ) {
		$class .= banquet_core_get_post_value_through_levels( 'qodef_' . $this->slug . '_header_in_grid' ) == 'yes' ? 'qodef-content-grid' : '';
	...
	public function set_inline_header_styles( $style ) {
		...
		$height           = banquet_core_get_post_value_through_levels( 'qodef_' . $this->slug . '_header_height' );
		$background_color = banquet_core_get_post_value_through_levels( 'qodef_' . $this->slug . '_header_background_color' );
		...
		$side_padding = banquet_core_get_post_value_through_levels( 'qodef_' . $this->slug . '_header_side_padding' );
