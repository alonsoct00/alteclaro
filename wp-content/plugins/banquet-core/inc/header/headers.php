...
class BanquetCoreHeaders {
	...
	public function __construct() {
		...
		add_action( 'banquet_action_after_page_header_inner', array( $this, 'scroll_appearance' ) );
	...
	function include_elements() {
		...
		foreach ( glob( BANQUET_CORE_INC_PATH . '/header/dashboard/*/*.php' ) as $admin ) {
			include_once $admin;
		}
		foreach ( glob( BANQUET_CORE_INC_PATH . '/header/layouts/*/include.php' ) as $layout ) {
			include_once $layout;
		}
		foreach ( glob( BANQUET_CORE_INC_PATH . '/header/scroll-appearance/*/include.php' ) as $appearance ) {
			include_once $appearance;
		}
		include_once BANQUET_CORE_INC_PATH . '/header/top-area/include.php';
	}

	function set_variables() {
		$layout_meta = banquet_core_get_post_value_through_levels( 'qodef_header_layout' );
		$layouts     = apply_filters( 'banquet_core_filter_register_header_layouts', $header_layouts_option = array() );
	...
	function render_template() {
		...
		if ( ! empty( $header_object ) ) {
			$template_hook = $header_object->overriding_whole_header ? 'banquet_filter_header_template' : 'banquet_filter_header_content_template';
	...
	function add_body_classes( $classes ) {
		$header_layout            = banquet_core_get_post_value_through_levels( 'qodef_header_layout' );
		$header_skin              = banquet_core_get_post_value_through_levels( 'qodef_header_skin' );
		$header_scroll_appearance = banquet_core_get_post_value_through_levels( 'qodef_header_scroll_appearance' );
	...
	function scroll_appearance() {
		...
		if ( ! empty( $header_object ) ) {
			$appearance_type = banquet_core_get_post_value_through_levels( 'qodef_header_scroll_appearance' );

			if ( file_exists( BANQUET_CORE_HEADER_LAYOUTS_PATH . '/' . $header_object->slug . '/templates/' . $appearance_type . '.php' ) ) {
				$scroll_appearance_layout = 'layouts/' . $header_object->slug;
			} else {
				$scroll_appearance_layout = 'scroll-appearance/' . $appearance_type;
			}

			banquet_core_template_part( 'header/' . $scroll_appearance_layout, 'templates/' . $appearance_type, '', array() );
		}
	}
	...
	function add_header_widget_areas() {
		register_sidebar(
			array(
				...
				'name'          => esc_html__( 'Header - Area One', 'banquet-core' ),
				'description'   => esc_html__( 'Widgets added here will appear in header widget area one', 'banquet-core' ),
		...
		register_sidebar(
			array(
				...
				'name'          => esc_html__( 'Header - Area Two', 'banquet-core' ),
				'description'   => esc_html__( 'Widgets added here will appear in header widget area two', 'banquet-core' ),
		...
		do_action( 'banquet_core_action_additional_header_widgets_area' );
	}
}

BanquetCoreHeaders::get_instance();