...
if ( ! function_exists( 'banquet_add_rest_api_pagination_global_variables' ) ) {
	function banquet_add_rest_api_pagination_global_variables( $global, $namespace ) {
	...
	add_filter( 'banquet_filter_rest_api_global_variables', 'banquet_add_rest_api_pagination_global_variables', 10, 2 );
}

if ( ! function_exists( 'banquet_add_rest_api_pagination_route' ) ) {
	function banquet_add_rest_api_pagination_route( $routes ) {
		$routes['pagination'] = array(
			'route'    => 'get-posts',
			'methods'  => WP_REST_Server::READABLE,
			'callback' => 'banquet_get_new_posts',
			'args'     => array(
				'options' => array(
					'required'          => true,
					...
					'description'       => esc_html__( 'Options data is array with all selected shortcode parameters value', 'banquet' )
				)
			)
		);
	...
	add_filter( 'banquet_filter_rest_api_routes', 'banquet_add_rest_api_pagination_route' );
}

if ( ! function_exists( 'banquet_get_new_posts' ) ) {
	/**
	 * Function that load new posts for pagination functionality
	 *
	 * @return void
	 */
	function banquet_get_new_posts() {
		
		if ( ! isset( $_GET ) || empty( $_GET ) ) {
			banquet_get_ajax_status( 'error', esc_html__( 'Get method is invalid', 'banquet' ) );
		} else {
			...
			if ( ! empty( $options ) ) {
				...
				$query_args = banquet_get_query_params( $options );
				...
				echo apply_filters( "banquet_filter_{$get_template_part}", $get_template_part( $module . '/' . $shortcode, 'templates/loop', '', $options ) );
				
				$html = ob_get_contents();
				
				ob_end_clean();
				
				banquet_get_ajax_status( 'success', esc_html__( 'Items are loaded', 'banquet' ), $html );
			} else {
				banquet_get_ajax_status( 'error', esc_html__( 'Options are invalid', 'banquet' ) );
			}
		}
	}
}

if ( ! function_exists( 'banquet_get_query_params' ) ) {
	/**
	 ...
	 * @return array
	 */
	function banquet_get_query_params( $params ) {
		...
		return apply_filters( 'banquet_filter_query_params', $args, $params );
	}
}

if ( ! function_exists( 'banquet_get_pagination_data' ) ) {
	/**
	 ...
	 * @return array
	 */
	function banquet_get_pagination_data( $plugin, $module, $shortcode, $post_type, $params ) {
		$data = array();
		...
		if ( ! empty( $post_type ) && ! empty( $params ) ) {
			...
			if ( isset( $params['space'] ) && ! empty( $params['space'] ) ) {
				$params['space_value'] = banquet_core_get_space_value( $params['space'] );
			}
...
if ( ! function_exists( 'banquet_add_link_pages_after_content' ) ) {
	/**
	 * Function which add pagination for blog single and page
	 */
	function banquet_add_link_pages_after_content() {
		
		$args_pages = array(
			'before'      => '<div class="qodef-e-single-links"><span class="qodef-e-single-links-title">' . esc_html__( 'Pages: ', 'banquet' ) . '</span>',
			'after'       => '</div>',
			'link_before' => '<span>',
			'link_after'  => '</span>',
			'pagelink'    => '%',
	...
	add_action( 'banquet_action_after_blog_single_content', 'banquet_add_link_pages_after_content' );
	add_action( 'banquet_action_after_page_content', 'banquet_add_link_pages_after_content' );
}
